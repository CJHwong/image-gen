...
def score(pil, label):
    ...
    g = a.mean(axis=2)
    print(
        f"{label:28s} mean {g.mean():6.1f}  contrast {g.std():5.1f}  "
        f"p1-p99 {np.percentile(g, 1):5.1f}-{np.percentile(g, 99):5.1f}",
        flush=True,
    )
...
for device, dtype, label in (
    ("mps", torch.bfloat16, "mps bf16"),
    ("mps", torch.float32, "mps fp32"),
    ("cpu", torch.float32, "cpu fp32"),
):
    vae = AutoencoderKLQwenImage21.from_pretrained("Qwen/Qwen-Image-2.1", subfolder="vae", torch_dtype=dtype).to(device)
