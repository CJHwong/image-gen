...
engine from qwen21_gen.py. mflux has not ported the Qwen3-VL vision tower, so it
...
What that buys, over the img2img in qwen21_gen.py:
...
def print_header(args, images):
    rows = [
        ...
        ("References", ", ".join(f"{p} ({im.size[0]}x{im.size[1]})" for p, im in zip(args.image, images))),
        ("Prompt", args.prompt),
...
def parse_stdio_job(request):
    ...
    if len(encoded) > MAX_REFERENCE_IMAGES:
        raise ValueError(
            f"{len(encoded)} references given, the model takes at most {MAX_REFERENCE_IMAGES}"
        )
    return {
...
def run_stdio():
    ...
    for line in sys.stdin.buffer:
        ...
        except Exception as error:  # noqa: BLE001 - the caller needs the reason, not a traceback
